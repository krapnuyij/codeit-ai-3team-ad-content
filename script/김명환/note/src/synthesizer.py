...
class ObjectSynthesizer:
    ...
    def _load_model(self, with_ip_adapter: bool = False, use_4bit: bool = True):
        """
        FLUX.1-Fill 파이프라인을 로드합니다.

        Args:
            with_ip_adapter: IP-Adapter를 함께 로드할지 여부
            use_4bit: 4bit 양자화 사용 여부 (기본값: True, L4 GPU에 최적화)
        """
        if self.pipe is None:
            print(f"  FLUX.1-Fill 파이프라인을 {self.device}에 로드 중...")

            if use_4bit:
                # L4 GPU를 위한 4bit 양자화 설정 (메모리 대폭 절약!)
            ...
            # 1단계: 베이스 트랜스포머를 양자화로 로드
            print(f"  트랜스포머 로드 중 ({quant_type} 양자화)...")
            ...
            self._print_gpu_memory(f"트랜스포머 로드 후 ({quant_type})")
            ...
            print(f"  ✓ FLUX.1-Fill 파이프라인 로드 완료 ({quant_type} 양자화)")
    ...
    def _load_flux_pipeline(self, use_4bit: bool = True):
        """
        ...
        if self.flux_pipe is None:
            ...
            if use_4bit:
                ...
                # 1단계: 트랜스포머를 4bit 양자화로 로드
                print(f"  트랜스포머 로드 중 (4bit 양자화)...")
                from diffusers.models import FluxTransformer2DModel
                ...
                self._print_gpu_memory("트랜스포머 로드 후 (4bit)")

                # 2단계: 양자화된 트랜스포머로 파이프라인 생성
                print(f"  파이프라인 생성 중 (4bit 트랜스포머 사용)...")
                self.flux_pipe = FluxPipeline.from_pretrained(
                    "black-forest-labs/FLUX.1-dev",
                    transformer=flux_transformer,
                    torch_dtype=self.torch_dtype,
                )
                self._print_gpu_memory("FluxPipeline 로드 후 (4bit)")
            else:
                # 기존 방식: 8bit 또는 양자화 없음
            ...
            if self.device == "cuda":
                ...
                print(f"  ✓ 메모리 최적화 활성화 (CPU offload + Attention slicing)")
                self._print_gpu_memory("최적화 적용 후")
            ...
            quant_info = "4bit" if use_4bit else "기본"
            print(f"  ✓ FluxPipeline + IP-Adapter 로드 완료 ({quant_info} 양자화)")
    ...
    def _stage1_ip_adapter_generation(
        ...
        self._load_flux_pipeline(use_4bit=use_4bit)
    ...
    def fill_in_object(
        ...
        Note:
            ...
            - use_4bit=True로 설정하면 L4 GPU(22GB)에서도 2단계 파이프라인 사용 가능!

        Example:
            >>> # 2단계 파이프라인 + 4bit 양자화 (L4 GPU 최적화, 권장!)
        ...
        try:
            ...
            if use_two_stage and self.enable_ip_adapter:
                # ===== 2단계 하이브리드 파이프라인 =====
                quant_msg = "4bit 양자화 (L4 최적화)" if use_4bit else "8bit 양자화"
                print(f"  ✓ 2단계 파이프라인 실행 ({quant_msg})")
                ...
                # 모델 로드
                self._load_model(with_ip_adapter=False, use_4bit=use_4bit)
