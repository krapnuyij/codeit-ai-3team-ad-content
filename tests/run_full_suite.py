...
def run_unit_tests():
    ...
    try:
        ...
        seg_result, seg_mask = seg_model.run(product_image)
        log_result(
            "1. Segmentation Result", "Success", seg_result, "Background removed"
        )
        log_result("1. Segmentation Mask", "Success", seg_mask, "Mask generated")
    except Exception as e:
        log_result(
            "1. SegmentationModel", "Error", None, f"{e}\n{traceback.format_exc()}"
        )
    ...
    try:
        ...
        log_result(
            "2. Flux Background", "Error", None, f"{e}\n{traceback.format_exc()}"
        )
    ...
    try:
        if seg_result and flux_bg:
            ...
            log_result(
                ...
                "Success",
                flux_refined,
                "Refined composite image with progress_callback",
            )
        else:
            log_result(
                "3. Flux Refinement",
                "Skipped",
                None,
                "Missing inputs (Segmentation or Flux BG)",
            )
    except Exception as e:
        log_result(
            "3. Flux Refinement", "Error", None, f"{e}\n{traceback.format_exc()}"
        )
    ...
    try:
        ...
        sdxl_text = sdxl_gen.generate_text_effect(
            canny_map, "Made of gold, shiny, metallic", "low quality"
        )
        log_result("4. SDXL Text Effect", "Success", sdxl_text, "3D Text Generated")
    except Exception as e:
        log_result(
            "4. SDXLTextGenerator", "Error", None, f"{e}\n{traceback.format_exc()}"
        )
    ...
    try:
        ...
        eng_seg, _ = engine.run_segmentation(product_image)
        log_result(
            "5. Engine Integration",
            "Success",
            eng_seg,
            "Engine.run_segmentation executed",
        )
    except Exception as e:
        log_result(
            "5. Engine Integration", "Error", None, f"{e}\n{traceback.format_exc()}"
        )
...
def run_fastapi_tests(reference_image=None):
    logger.info(
        "Testing FastAPI... (Mocking GPU tasks usually, but here we run real inference if possible or mocked)"
    )
    ...
    if job_id:
        ...
        if final_result_b64:
            ...
            log_result(
                "9. API Final Result", "Missing", None, "No final result returned"
            )
