...
class ImageBox(BaseModel):
    ...
    type: str = Field(
        default="", description="이미지 타입: hero_image, product_card, gallery_item 등"
    )
...
class ButtonElement(BaseModel):
    ...
    type: str = Field(
        default="", description="버튼 스타일: primary, secondary, ghost 등"
    )
    action: str = Field(
        default="",
        description="버튼 클릭 시 동작: navigate_to_menu, open_reservation_form 등",
    )
...
class TextBlock(BaseModel):
    ...
    type: str = Field(
        default="", description="텍스트 타입: headline, subheading, body, tagline 등"
    )
    content: str = Field(default="", description="실제 텍스트 내용")
    emphasis: str = Field(
        default="", description="강조 스타일: bold, italic, highlight 또는 빈 문자열"
    )
...
class SectionContent(BaseModel):
    ...
    section_name: str = Field(
        default="",
        description="어느 섹션의 콘텐츠인지 (PageVariations의 sections와 매칭)",
    )
    text_blocks: List[TextBlock] = Field(
        default_factory=list, description="섹션 내 텍스트 요소들"
    )
    images: List[ImageBox] = Field(
        default_factory=list, description="섹션 내 이미지 요소들"
    )
    buttons: List[ButtonElement] = Field(
        default_factory=list, description="섹션 내 버튼 요소들"
    )
...
class PageContent(BaseModel):
    ...
    sections: List[SectionContent] = Field(
        default_factory=list, description="섹션별 상세 콘텐츠"
    )
...
class DOMContract(BaseModel):
    ...
    page_wrapper: str = Field(
        default="div.page[data-page='{page_name}']", description="페이지 최상위 래퍼"
    )
...
class AdState(TypedDict):
    ...
    # 이미지 생성 결과 (LLM MCP 통합)
    generated_images: Dict[str, str]  # key=이미지ID (page_section_type), value=Base64
