...
class Nodes:
    ...
    def _node_bs_parser(self, state:AdState) ->Dict[str, Any]:
        return asyncio.run(create_bs_parser(state, self.settings, self.prompts, self.client))

    def _node_concept_designer(self, state: AdState) -> Dict[str, Any]:
        return asyncio.run(create_concept_designer(state, self.settings, self.prompts, self.client))

    def _node_marketing_strategy(self, state: AdState) ->Dict[str, Any]:
        return asyncio.run(create_marketing_strategy(state, self.settings, self.prompts, self.client))

    def _node_homepage_designer(self, state: AdState) -> Dict[str, Any]:
        return asyncio.run(create_homepage_designer(state, self.settings, self.prompts, self.client))

    def _node_content_designer(self, state: AdState) -> Dict[str, Any]:
        return asyncio.run(create_content_designer(state, self.settings, self.prompts, self.client))

    def _node_dom_contract(self, state: AdState) -> Dict[str, Any]:
        return asyncio.run(create_dom_contract(state, self.settings, self.prompts, self.client))

    def _node_validate_consistency(self, state: AdState) -> Dict[str, Any]:
        return asyncio.run(validate_consistency(state, self.settings, self.prompts, self.client))

    def _node_implementation_spec(self, state: AdState) -> Dict[str, Any]:
        return asyncio.run(create_implementation_spec(state, self.settings, self.prompts, self.client))

    def _node_header_footer(self, state: AdState) -> Dict[str, Any]:
        return asyncio.run(generate_header_footer(state, self.settings, self.prompts, self.client))

    def _node_html_code(self, state: AdState) -> Dict[str, Any]:
        return asyncio.run(generate_html_code(state, self.settings, self.prompts, self.client))

    def _node_css_code(self, state: AdState) -> Dict[str, Any]:
        return asyncio.run(generate_css_code(state, self.settings, self.prompts, self.client))
