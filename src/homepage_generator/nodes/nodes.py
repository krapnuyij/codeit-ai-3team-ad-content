...
import json
from agent_framework import AgentRunEvent, AgentRunUpdateEvent, WorkflowOutputEvent, ChatMessage
...
# Node 0: Brain Storm
async def create_brain_storm(state: AdState, settings: Settings, prompts:PromptsConfig) -> Dict[str, Any]:
    """
    ...
    client = OpenAIChatClient(model_id=settings.openai_config.chat_model, api_key=settings.openai_config.api_key)
    # 1. 사용할 에이전트 키 리스트 정의
    # agent_keys = ["sns_bs_agent", "blog_bs_agent", "design_bs_agent", "developer_bs_agent"]
    agent_keys = ["pm_agent", "designer_agent", "developer_agent", "content_agent", "expander_agent"]
    ...
    state["brainstorm_user_input"] = prompts.user_prompt.format(StoreInfo=state["store_info"])
    ...
    return {"brainstorm": brainstorm_text,
            "log": ["[BrainStorm Node] 브레인스토밍 완료"]}
# BS Parser Node
async def create_bs_parser(state: AdState, settings: Settings, prompts:PromptsConfig, client:AsyncOpenAI) -> Dict[str, Any]:
...
async def create_concept_designer(state: AdState, settings: Settings, prompts:PromptsConfig, client:AsyncOpenAI) -> Dict[str, Any]:
    """
    ...
    store_concept = await agent.create_structured(user_prompts,response_model=StoreConcept)
    print(store_concept)
...
async def create_marketing_strategy(state: AdState, settings: Settings, prompts:PromptsConfig, client:AsyncOpenAI) -> Dict[str, Any]:
    """
    ...
    user_prompts = agent_config.user_message.format(**store_info.model_dump(), **store_concept.model_dump(),)
    # response = await agent.create(message=user_prompts)
    #
    # result_text = response.choices[0].message.content
    # result_json = json.loads(result_text)
    ...
    marketing_strategy = await agent.create_structured(user_prompts,response_model=MarketingStrategy)
...
async def create_homepage_designer(state: AdState, settings: Settings, prompts:PromptsConfig, client:AsyncOpenAI) -> Dict[str, Any]:
    """
    ...
    homepage_design = await agent.create_structured(user_prompts,response_model=HomePageDesign)
...
async def create_content_designer(state: AdState, settings: Settings, prompts:PromptsConfig, client:AsyncOpenAI) -> Dict[str, Any]:
    """
    ...
    homepage_design_json = json.dumps(
        homepage_design.model_dump(),
        indent=2,
        ensure_ascii=False
    )

    user_prompts = agent_config.user_message.format(homepage_design=homepage_design_json, **store_info.model_dump())
    content_design = await agent.create_structured(user_prompts, response_model=HomePageDesign)
...
def generate_dom_contract_rule_based(content_design: HomePageDesign) -> DOMContract:
    ...
    dom_contract = DOMContract(
        text_mappings=text_mappings if text_mappings else {
...
async def create_dom_contract(state: AdState, settings: Settings, prompts: PromptsConfig, client: AsyncOpenAI) -> Dict[str, Any]:
    """
    ...
    return {
        ...
        "logs": [f"[DOM Contract] 룰 기반 생성 완료 (text mappings: {text_type_count}개)"]
    }


# # Node F: 구현 명세 생성(DEPRECATED)
async def create_implementation_spec(state: AdState, settings: Settings, prompts:PromptsConfig, client:AsyncOpenAI) -> Dict[str, Any]:
    """
...
async def generate_header_footer(state: AdState, settings: Settings, prompts:PromptsConfig, client:AsyncOpenAI) -> Dict[str, Any]:
    """
    ...
    colors_json = json.dumps(
        design_system.color_palette.model_dump(),
        indent=2,
        ensure_ascii=False
    )

    fonts_json = json.dumps(
        design_system.fonts.model_dump(),
        indent=2,
        ensure_ascii=False
    )

    header_json = json.dumps(
        design_system.header.model_dump(),
        indent=2,
        ensure_ascii=False
    )

    footer_json = json.dumps(
        design_system.footer.model_dump(),
        indent=2,
        ensure_ascii=False
    )
    ...
    return {
        ...
        "logs": [f"[Header/Footer] 공통 Header/Footer 생성 완료 (Header: {len(header_html)}자, Footer: {len(footer_html)}자)"]
    }


# Node G: Main 콘텐츠 생성 (Tailwind 방식)
async def generate_html_code(state: AdState, settings: Settings, prompts:PromptsConfig, client:AsyncOpenAI) -> Dict[str, Any]:
    """
    ...
    return {
        ...
        "logs": [f"[Main Content (Tailwind)] {len(html_codes)}개 페이지 main 콘텐츠 생성 완료 (총 길이: {total_length} 문자)"]
    }


# Node H: CSS 코드 생성 (DEPRECATED)
async def generate_css_code(state: AdState, settings: Settings, prompts:PromptsConfig, client:AsyncOpenAI) -> Dict[str, Any]:
    """
    ...
    html_codes_truncated_str = json.dumps(truncated_html_codes, indent=2, ensure_ascii=False)
...
async def evaluate_code_value(state: AdState, settings: Settings, prompts:PromptsConfig, client:AsyncOpenAI) -> Dict[str, Any]:
...
async def package_output(state: AdState, settings: Settings, prompts:PromptsConfig) -> Dict[str, Any]:
    """
