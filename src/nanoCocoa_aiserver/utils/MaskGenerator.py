...
class MaskGenerator:
    ...
    def create_text_alpha_mask(
        background: Image.Image, text_asset: Image.Image, dilation: int = 10
    ) -> Image.Image:
        """
    ...
    def create_empty_space_mask(
        background: Image.Image, threshold_percentile: int = 40, min_area: float = 0.15
    ) -> Image.Image:
        """
        ...
        if mask_ratio < min_area:
            logger.warning(
                f"Empty space too small ({mask_ratio:.2%}), using top position fallback"
            )
    ...
    @staticmethod
    def visualize_mask(
        background: Image.Image, mask: Image.Image, alpha: float = 0.5
    ) -> Image.Image:
        """
        마스크 시각화 (디버깅용)
    ...
    def recommend_position(
        ...
        scores = {"top": top_area, "center": center_area, "bottom": bottom_area}
        ...
        if scores[best_position] / (total_pixels / 3) < min_area:
            logger.warning(
                f"Not enough empty space found (max {scores[best_position]}), defaulting to top"
            )
            return "top"
