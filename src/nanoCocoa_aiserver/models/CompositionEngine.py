...
class CompositionEngine:
    ...
    def compose(
        ...
        try:
            ...
            # 2. 프롬프트 생성
            composition_prompt = self._build_composition_prompt(mode, position, user_prompt)
            logger.info(f"📝 Composition prompt: {composition_prompt}")
            
            # 네거티브 프롬프트 설정
            default_negative = "floating, disconnected, unrealistic, bad composition, low quality"
    ...
    def compose_simple(
        ...
        Returns:
            Image.Image: 합성된 이미지
        """
        logger.warning("⚠️ Using fallback simple composition (alpha composite)")
