...
try:
    ...
    logger.warning(
        "Diffusers/Transformers 라이브러리를 찾을 수 없습니다. AI 기능이 비활성화됩니다."
    )
    AI_AVAILABLE = False
...
class SDXLGenerator:
    ...
    def ensure_size(
        self, img: Image.Image, size: Tuple[int, int] = (1024, 1024)
    ) -> Image.Image:
    ...
    def sdxl_generate_background(
        ...
        out = (
            pipe(prompt=prompt, negative_prompt=full_neg, generator=g, **params)
            .images[0]
            .convert("RGB")
        )
    ...
    def ai_harmonization_pro(
        ...
        mask_canvas = mask_canvas.filter(ImageFilter.MaxFilter(15)).filter(
        ...
        try:
            ...
            if user_bg_path and os.path.exists(user_bg_path):
                ...
                if user_bg_mode == "reuse":
                    ...
                    regen_prompt = self.build_regen_prompt(
                        cap, self.palette_to_prompt_text(palette)
                    )
                    ...
                    if progress_callback:
                        progress_callback(50, "고화질 배경 재생성 중...")
                    bg = self.sdxl_generate_background(
                        regen_prompt, seed, bg_user.size, params
                    )
                    self.unload()
                ...
                if progress_callback:
                    progress_callback(30, f"배경 생성 중 (User Prompt 적용)...")
                bg = self.sdxl_generate_background(
                    prompt_text, seed, (1024, 1024), params
                )
                self.unload()
            ...
            if use_ai_gen and AI_AVAILABLE:
                if progress_callback:
                    progress_callback(70, "AI 조명 및 그림자 합성 중...")
                ...
                shadow = mask.resize((fg_res.width, shadow_h)).filter(
...
def main():
    ...
    parser.add_argument(
        "--input",
        type=str,
        default="outputs/fg_cut",
        help="입력(누끼) 이미지 폴더 경로",
    )
    parser.add_argument(
        "--mask",
        type=str,
        default="outputs/fg_mask",
        help="마스크 이미지 폴더 경로 (선택)",
    )
    parser.add_argument(
        "--output", type=str, default="outputs/compose", help="결과물 저장 폴더 경로"
    )
