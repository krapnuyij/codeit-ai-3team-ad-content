...
Create a single FLUX.2 prompt that maintains product authenticity while applying the requested edits."""
        ...
        # OpenAI API 호출
        logger.debug(
            f"OpenAI API 호출: model={self.model}, max_tokens={self.max_completion_tokens}"
        )
        response = self.openai_client.chat.completions.create(
            model=self.model,
            messages=messages,
        ...
        logger.debug(
            f"API 응답: finish_reason={finish_reason}, tokens={response.usage.total_tokens}"
        )
        ...
        if finish_reason == "length":
            logger.warning(
                f"프롬프트가 max_tokens 제한으로 잘렸을 수 있음: {self.max_completion_tokens}"
            )
