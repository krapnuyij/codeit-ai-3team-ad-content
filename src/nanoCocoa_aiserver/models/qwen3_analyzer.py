...
class Qwen3Analyzer:
    """
    Qwen VL을 사용하여 배경 이미지에서 최적의 객체 배치 위치를 찾는 클래스
    ...
    def __init__(self, model_name: str = MODEL_IDS["QWEN3"], device: str = DEVICE):
        """
        Qwen3Analyzer 초기화
        ...
        logger.debug(f"Qwen3Analyzer 초기화: {model_name}")

    def unload(self) -> None:
        """명시적 리소스 정리 (표준 인터페이스)"""
        log_gpu_memory("Qwen3Analyzer unload (before)")
        self._unload_model()
        log_gpu_memory("Qwen3Analyzer unload (after)")

        logger.info("Qwen3Analyzer unloaded")
