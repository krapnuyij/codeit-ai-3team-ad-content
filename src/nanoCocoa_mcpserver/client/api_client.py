...
class AIServerClient:
    ...
    async def _request(
        ...
        **kwargs
        ...
        while retry_count <= MAX_RETRIES_ON_503:
            try:
                logger.debug(f"{method} {endpoint} 요청 (시도 {retry_count + 1}/{MAX_RETRIES_ON_503 + 1})")
                ...
                raise AIServerError(
                    f"AI 서버에 연결할 수 없습니다: {self.base_url}",
                    detail=str(e)
                )
    ...
    async def start_generation(self, params: GenerateRequest) -> GenerateResponse:
        ...
        """
        logger.info(f"생성 시작: start_step={params.start_step}, text={params.text_content}")
        ...
        while retry_count < max_retries:
            try:
                ...
                if status.status == "completed":
                    logger.info(f"작업 완료: job_id={job_id}, 소요 시간={status.elapsed_sec:.1f}초")
                    return status
