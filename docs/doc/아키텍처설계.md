---
layout: default
title: "AI 광고 콘텐츠 생성 시스템 아키텍처 설계서"
description: "AI 광고 콘텐츠 생성 시스템 아키텍처 설계서"
date: 2026-01-02
author: "김명환"
...
- **사용자 친화성**: Streamlit 기반의 직관적인 UI로 디자인 역량이 부족한 소상공인도 쉽게 사용
...
| 계층 | 기술 스택 | 비고 |
|-----|----------|------|
| **프론트엔드** | Streamlit, Python 3.11+ | 사용자 인터페이스 |
...
graph TB
    ...
    subgraph "프론트엔드 계층 (Docker Container 1)"
        Frontend["Streamlit UI<br/>FastAPI 기반<br/>Port: External"]
    end
...
sequenceDiagram
    ...
    participant FE as 프론트엔드<br/>(Streamlit)
...
graph TB
    subgraph "GCP VM Instance (L4 GPU)"
        subgraph "Docker Network: ad-content-network"
            Container1["Container 1<br/>Frontend<br/>Streamlit + FastAPI<br/>Port: 외부 노출"]
            Container2["Container 2<br/>Backend<br/>FastAPI<br/>Port: 8080 (내부)"]
            Container3["Container 3<br/>Model Serving<br/>FastAPI + AI Models<br/>Port: 8000 (내부)"]
        end

        GPU_Device["NVIDIA L4 GPU<br/>Device: /dev/nvidia0"]
    end
...
services:
  frontend:
    build: ./frontend
    container_name: ad-content-frontend
    ports:
      - "80:8501"  # Streamlit 기본 포트
    networks:
      - ad-content-network
    environment:
  ...
  backend:
    build: ./backend
    container_name: ad-content-backend
    ports:
  ...
  model-serving:
    ...
    container_name: ad-content-model-serving
    ports:
...
| **모델서빙** | 8000 | 8000 | Internal Only | AI 모델 추론 API |
