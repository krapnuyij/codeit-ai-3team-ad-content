...
1.1. [아키텍처 개요](#11-아키텍처-개요)<br/>
1.2. [필수 요구사항](#12-필수-요구사항)<br/>
1.3. [설치](#13-설치)<br/>
1.4. [FastAPI 서버 실행](#14-FastAPI-서버-실행)<br/>
1.5. [테스트](#15-테스트)
1.6. [API 사용 예시](#16-api-사용-예시)<br/>
1.7. [문제 해결](#17-문제-해결)<br/>
...
**증상**: LLM Desktop에서 도구가 보이지 않음

**해결방법**:
1. FastAPI 서버가 `http://localhost:8000`에서 실행 중인지 확인
2. `LLM_desktop_config.json`에서 절대 경로 확인
3. LLM Desktop 로그 확인:
   - Windows: `%APPDATA%\LLM\logs`
   - macOS: `~/Library/Logs/LLM`
4. LLM Desktop 재시작
