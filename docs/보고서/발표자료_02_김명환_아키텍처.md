...
3. [모델 서버 구축](#3-모델-서버-구축)
4. [AI 모델 파이프라인](#4-ai-모델-파이프라인)
5. [GPU 메모리 최적화](#5-gpu-메모리-최적화)
6. [Docker 배포 환경](#6-docker-배포-환경)
7. [API 설계](#7-api-설계)
...
graph TB
    ...
    subgraph ModelServing["모델 서빙 계층 (Port: 8892)"]
...
# 외부에서는 호스트 IP:포트로 접근
# http://34.44.205.198:8892/generate
```
...
graph TB
    subgraph Host["GCP VM Host<br/>Ubuntu 22.04 LTS<br/>NVIDIA L4 GPU 24GB"]
        ...
        subgraph Network["Docker Network: nanococoa-network<br/>(bridge driver)"]
            ...
            AIServer["nanococoa_aiserver<br/>(8892)"]
...
# ❌ 잘못된 방법: 호스트 IP 사용
AISERVER_URL = "http://34.44.205.198:8892"
...
| nanococoa_aiserver | 8000 | 8892 | AI 모델 API |
