...
| 비동기 (Asynchronous) | Job ID + Polling | - 이미지 생성<br>- 홈페이지 생성 | 처리 시간 7~8분 |
...
- **도구 기반**: LLM이 직접 호출할 수 있는 5개 함수 제공
- **확장 가능**: 새로운 도구 추가 용이
...
| 도구명 | 설명 | 주요 파라미터 | 반환값 |
|--------|------|----------|--------|
| `generate_ad_image` | 광고 이미지 생성 (3단계 파이프라인) | product_image_path, background_prompt, text_content, text_prompt, bg_model | job_id, status |
| `check_generation_status` | 생성 상태 확인 | job_id | progress, message, result |
| `stop_generation` | 생성 작업 중단 | job_id | stopped |
| `list_available_fonts` | 사용 가능 폰트 목록 | - | fonts[] |
| `check_server_health` | 서버 상태 확인 | - | gpu_info, cpu_percent, ram_used |

**도구 상세 설명**

1. **generate_ad_image**: 가장 핵심 도구
   - 3단계 파이프라인: 배경 생성 → 3D 텍스트 생성 → 합성
   - bg_model: 'flux'(고품질, 기본값) 또는 'sdxl'(빠른 속도)
   - stop_step: 1(배경만), 2(배경+텍스트), 3(전체)
   - wait_for_completion: True(대기) 또는 False(job_id만 반환)

2. **check_generation_status**: 비동기 작업 모니터링
   - 진행률, 현재 단계, 예상 시간 제공
   - 완료 시 결과 이미지 Base64 반환

3. **stop_generation**: 긴 작업 중단
   - 7~8분 소요되는 작업을 중간에 중단 가능

4. **list_available_fonts**: 폰트 탐색
   - 한글/영문 폰트 목록 제공
   - generate_ad_image의 font_name 파라미터에 사용

5. **check_server_health**: 서버 모니터링
   - GPU 메모리, CPU, RAM 사용률 확인
   - 큰 작업 시작 전 리소스 확인용
...
LLM: [generate_ad_image 도구 호출]
     - product_image_path: [<업로드된 사과 이미지 경로>]
     - background_prompt: "Korean traditional market, wooden table..."
     - text_content: "명절 특가! 한 박스 20,000원"
     - text_prompt: "Bold Korean text, 3D style, golden color"
     - bg_model: "flux"
...
**NVIDIA L4 GPU 기준 실측 벤치마크** (2026-01-27 실측)

| 단계 | 작업 | 시간 | VRAM | 비고 |
|------|------|------|------|------|
| **Step 1: 배경 생성** | | | | |
| 1-1 | FLUX.1-dev 로드 | 128초 | 11GB | 8bit 양자화 (첫 실행) |
| 1-2 | FLUX 추론 | 133초 | 11GB | 25 steps, img2img |
| 1-3 | FLUX 언로드 | 16초 | - | 메모리 정리 |
| **Step 1 소계** | | **~277초** | | **약 4분 37초** |
| **Step 2: 텍스트 생성 (LLM 통합)** | | | | |
| 2-1 | Step 1 모델 언로드 | 2초 | - | GPU 메모리 확보 |
| 2-2 | Qwen2-VL 로드 | 73초 | 10GB | 5 shards 로딩 |
| 2-3 | Qwen 이미지 분석 | 58초 | 10GB | 공간/색감/조명 분석 |
| 2-4 | Qwen 언로드 | 2초 | - | 메모리 해제 |
| 2-5 | LLM HTML 생성 | 44초 | - | OpenAI GPT-5-mini API |
| 2-6 | Playwright 렌더링 | 7초 | - | HTML → PNG |
| **Step 2 소계** | | **~186초** | | **약 3분 6초** |
| **전체 합계** | | **~463초** | | **약 7분 43초** |

**참고**: 평균 실행 시간 (통계 기록)
- Step 1 평균: 263초 (25.8회 실행 평균)
- Step 2 평균: 207초 (29.99회 실행 평균)  
- 전체 평균: **약 470초 (7분 50초)**
